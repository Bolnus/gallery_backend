...
function getTagsWithLocalePipeline(locale?: string): mongoose.PipelineStage[] {
  const albumWithCoverVar = "$albumWithCover";
  return [
    ...
    // Stage 3: Calculate counts and find first album with cover
    {
      $addFields: {
        ...
        filteredAlbumsCount: { $size: albumWithCoverVar },
        // Albums with cover pictures
        albumsWithCover: {
          $filter: {
            input: albumWithCoverVar,
            as: "album",
            cond: "$$album.hasCover"
          }
        },
...
export async function getTagsWithCoverLocale(locale?: string): Promise<TagItemWithCover[]> {
  try {
    const result = await TagsModel.aggregate(getTagsWithLocalePipeline(locale));
...
export async function getTagsWithCoverLocalePerformance(locale?: string): Promise<TagItemWithCover[]> {
  try {
    const result = await TagsModel.aggregate(getTagsWithLocalePipeline(locale)).explain();
    timeLog(JSON.stringify(result, null, 2));
